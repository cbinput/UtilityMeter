    ...
    : ICondominiumReconciliationRepository
{
    ...
    {
        var condominium = await this.context.Condominiums
            .AsNoTracking()
            .FirstOrDefaultAsync(item => item.Id == condominiumId, cancellationToken);
        if (condominium == null)
        {
            return null;
        }
        ...
        var meterIds = properties.SelectMany(property => property.MeterIds)
            ...
            .Distinct()
            .ToList();
        var meters = await this.context.Meters
            .AsNoTracking()
            .Where(meter => meterIds.Contains(meter.Id))
            .ToListAsync(cancellationToken);
        var readings = await this.context.Readings
            .AsNoTracking()
            .Where(reading => meterIds.Contains(reading.MeterId))
            .ToListAsync(cancellationToken);
